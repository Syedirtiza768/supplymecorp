...
export default function ProductCard({
  ...
  return (
    <Card className="overflow-hidden transition-all hover:shadow-md">
      <Link href={`/product/${id}`} className="block group" tabIndex={-1} style={{ textDecoration: 'none', color: 'inherit' }}>
        ...
        <CardContent className="p-4">
          <div className="mb-1 text-xs text-muted-foreground">
            <span className="hover:text-primary">{subcategoryName}</span>
          </div>
          <h3 className="line-clamp-2 font-medium group-hover:text-primary">{name}</h3>
      ...
      <CardFooter className="p-4 pt-0 flex gap-2">
        <Button 
          ...
          className="flex-1" 
          tabIndex={0} 
          onClick={(e) => { e.preventDefault(); window.location.href = `/product/${id}`; }}
        >
          View Details
        </Button>
        <Button className="flex-1 bg-primary hover:bg-primary-700" disabled={!inStock} tabIndex={0} onClick={e => { e.stopPropagation(); /* add to cart logic here */ }}>
